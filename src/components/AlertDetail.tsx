...
export function AlertDetail({ alert, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 sm:p-6"
      style={{ background: 'rgba(8,4,18,0.85)', backdropFilter: 'blur(16px)' }}
      onClick={onClose}>
      <div className="animate-modal w-full max-w-xs rounded-3xl p-5"
        style={{ background: '#0f0a1e', border: `1px solid ${alert.color}44` }}
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <div className="font-head font-bold text-sm leading-tight">{alert.title}</div>
          </div>
        </div>

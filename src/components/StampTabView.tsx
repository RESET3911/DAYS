...
export function StampTabView({ stampsByDate, onToggle }: Props) {
  const today = todayStr();
  const todayStamps = stampsByDate[today] || [];
  const dates = recentDates(14);
  const displayStamps = STAMPS;
          ...
          {displayStamps.map(({ emoji, label }) => {
            ...
            return (
              <button key={emoji} onClick={() => onToggle(today, emoji)} title={label}
                className="flex flex-col items-center rounded-xl py-1.5 gap-0.5 transition-all active:scale-90"
                style={{
